...
const statusStyles: Record<string, { bg: string; glow: string }> = {
  DRAFT: { bg: "bg-[var(--color-border-primary)]", glow: "" },
...
export default function MissionCard({ mission }: { mission: Mission }) {
  ...
  const status = statusStyles[mission.status] || { bg: "bg-[var(--color-border-primary)]", glow: "" };
  const borderColor = priorityBorders[mission.priority] || "border-l-military-500";

  return (
    <div
      onClick={() => navigate(`/missions/${mission.id}`)}
      className={`group relative glass-panel border border-[var(--color-border-primary)] rounded-xl p-5 cursor-pointer
        transition-all duration-300 ease-out
        hover:border-[var(--color-border-primary)] hover:scale-[1.02] hover:shadow-lg hover:shadow-black/20
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <h3 className="font-semibold text-base text-[var(--color-text-primary)] group-hover:text-tactical-500 transition-colors truncate pr-2">
          {mission.name}
        </h3>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase tracking-wider
          ${mission.priority === "CRITICAL" ? "bg-danger-500/20 text-danger-500" :
            ...
            "bg-[var(--color-bg-elevated)]/80 text-[var(--color-text-secondary)]"}`}>
          {mission.priority}
        </span>
      </div>
      ...
      <div className="space-y-2 text-sm">
        ...
        {mission.scheduledStart && (
          <div className="flex items-center gap-1.5 text-[var(--color-text-secondary)] text-xs font-mono">
