...
export default function MissionFilters({ filters, onChange }: Props) {
  ...
  const selectClasses = `bg-[var(--color-bg-elevated)]/80 border border-[var(--color-border-primary)] rounded-lg px-3 py-2 text-sm
    text-[var(--color-text-primary)] focus:outline-none focus:border-command-500/50 focus:ring-1 focus:ring-command-500/20
    transition-colors cursor-pointer appearance-none min-h-[44px]`;

  return (
    <div className="glass-panel rounded-xl border border-[var(--color-border-subtle)] p-4 mb-6">
      ...
      <div className="flex flex-col md:flex-row flex-wrap gap-3 items-stretch md:items-center">
        <div className="relative flex-1 min-w-0 md:min-w-[220px]">
          ...
          <input
            ...
            className="w-full bg-[var(--color-bg-elevated)]/80 border border-[var(--color-border-primary)] rounded-lg pl-9 pr-3 py-2 text-sm
              placeholder-[var(--color-input-placeholder)] text-[var(--color-text-primary)] min-h-[44px]
